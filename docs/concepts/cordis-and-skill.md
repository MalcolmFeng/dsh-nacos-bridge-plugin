...
- **但模型不一定知道什么时候用、怎么用才对**。例如某个异步工具要先提交拿单号、再凭单号查结果，不能同轮反复轮询——这类"流程知识"通常写在工具 description 之外，属于编排约定，需要 Skill 补充。
...
```markdown
## 异步任务流转（是否异步以工具 description 为准，不要臆造命名）
1. 提交类工具返回单号 → 告诉用户已受理，不要同轮反复轮询
2. 用户回访时凭单号调用查询类工具 → 拿到终态再整合输出
```
